...
// --- Fix ResearchFeature href ---
write(
  'src/components/editorial/ResearchFeature.tsx',
...
// Fix search research hrefs
const searchPath = 'src/lib/cms/search.ts';
let searchSrc = readFileSync(searchPath, 'utf8');
searchSrc = searchSrc.replace(
  /href: `\/research\/\$\{project\.researchStatus === 'ongoing' \? 'ongoing' : 'previous'\}\/\$\{project\.slug\}`/,
...
writeFileSync(searchPath, searchSrc);
console.log('✓', searchPath, '(hrefs)');
    ...
    return (
      <div className="border border-accent/30 bg-sage/40 px-6 py-10 md:px-8">
        ...
        <p className="mt-3 text-base leading-relaxed text-muted">
          Your message has been recorded in this demo. For a live enquiry, please
          email BKSR directly using the addresses on this page.
        </p>
        <Button
          type="button"
          variant="secondary"
