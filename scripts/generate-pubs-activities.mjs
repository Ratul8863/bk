        ...
        <Container>
          <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_16rem]">
            <article>
              ...
              {item.doi ? (
                <p className="mt-8 text-sm text-muted">DOI: {item.doi}</p>
              ) : null}
              {item.url ? (
            ...
            <aside className="space-y-6 border-t border-border pt-6 lg:sticky lg:top-28 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-6 lg:self-start">
              <div>
...
const primaryRoutes = ACTIVITY_ROUTE_META.filter((item) =>
...
import { Section } from '@/components/ui/Section';
import { RichText } from '@/components/ui/RichText';
import { getActivities, getEventById } from '@/lib/content/queries';
import { buildPageMetadata } from '@/lib/seo/metadata';
import { ACTIVITY_ROUTE_META } from '@/lib/public/labels';
import { formatDate } from '@/lib/utils';
// formatDate alias used below
...
console.log('publications + activities done');
